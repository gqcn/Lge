...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
	...
	protected function _save($fp, $dir, $name, $mime, $w, $h) {
		...
		while (!feof($fp)) {
		    ...
		    if ($quota <= 0) {
		        ...
		        fwrite($target, fread($fp, $persize));
		    }
		}
		fclose($target);
		$userModule->updateQuota($quota);
		@chmod($path, $this->options['fileMode']);
		clearstatcache();
		$this->_clearThumb($path);
		return $path;
	...
	/**
	 * 清除指定图像文件的缩略图(图片文件在上传、移动、删除的时候需要调用该函数).
	 * 
	 * @param string $filePath 图像文件的绝对路径.
	 */
	protected function _clearThumb($filePath)
	{
	    if (Lib_FileSys::isImage($filePath)) {
	        ...
	        foreach ($files as $file) {
	            ...
	            $path = $dirPath.'/'.$file;
	            // var_dump("/{$preg}\.(\d+)x(\d+)\.(.+)/i");exit();
	            if (preg_match("/{$preg}\.(\d+)x(\d+)\.(.+)/i", $path)) {
	                @unlink($path);
	            }
	        }
	    }
